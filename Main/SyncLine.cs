...
namespace BandParty.Main
{
    public class SyncLine : MonoBehaviour
    {
        ...
        void Update()
        {
            if (!this.note1.isActive || !this.note2.isActive || Mathf.Abs(note1.time - note2.time) * 1000 >= 1 || CalcTime(true) >= this.manager.speed + this.manager.afterTime || CalcTime(false) >= this.manager.speed + this.manager.afterTime)
            {
        ...
        {
            return soundManager.time - ((isNote1 ? this.note1.time : this.note2.time) - this.manager.speed);
        }
    }
